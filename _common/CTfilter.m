function [w_update,m_update,P_update,L_update,LK] = CTfilter(k,model,meas,est,filter,w_update,m_update,P_update,L_update)
%---prediction 
    [m_predict,P_predict] = CT_ekf_predict(model,m_update,P_update);                          %surviving components
    ...
    %---gating
    if filter.gate_flag
        meas.Z{k}= CT_gate_meas(meas.Z{k},filter.gamma,model,m_predict,P_predict);        
    end
    ...
    if m~=0
        %m detection terms 
        [qz_temp,m_temp,P_temp] = CT_ekf_update(meas.Z{k},model,m_predict,P_predict);
        for ell=1:m
    ...
    %Likelihood
    Z = meas.Z{k};
    LK = CT_compute_likelihood(k,model,Z,est,m_predict,P_predict);
    %est.Lk
    ...
    for j=1:length(idx)
        ...
        est.W{k,2}= [ est.W{k,2}; repmat(w_update(idx(j),:),[repeat_num_targets,1]) ];
        est.X{k,2}= [ est.X{k,2} repmat(m_update(:,idx(j)),[1,repeat_num_targets]) ];
...
function LK = CT_compute_likelihood(k,model,Z,est,m_predict,P_predict)
